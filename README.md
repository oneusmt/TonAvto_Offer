...
Все API теперь защищены токеном. Чтобы получить доступ:

1. Отправьте `POST /api/auth/login` с телом:
   ```json
   {
     "username": "oneusmt",
     "password": "da0xF-OzM_jMev"
   }
   ```
2. В ответе вернётся `token`. Передавайте его в каждом запросе в заголовке `Authorization: Bearer <token>`.
3. Для завершения сессии можно вызвать `POST /api/auth/logout` (также с заголовком `Authorization`).
...
#### Sold (Продажи)
- `GET /api/sold` - Получить все продажи
- `POST /api/sold` - Создать продажу
- `PUT /api/sold/{id}` - Обновить продажу
- `DELETE /api/sold/{id}` - Удалить продажу

> ⚠️ Все перечисленные запросы требуют действующего Bearer-токена.

### Веб-интерфейс

1. Откройте `http://localhost:5173` в браузере
2. Авторизуйтесь, указав логин `oneusmt` и пароль `da0xF-OzM_jMev`
3. После входа переключайтесь между вкладками "Предложения", "Покупки" и "Продажи"
4. Используйте кнопки для создания, редактирования и удаления записей
